# GeneForge — Genetic Programming Engine That Evolves JavaScript Programs
...
GeneForge is a genetic programming engine that breeds JavaScript programs through Darwinian evolution. Given input-output examples, it evolves a population of candidate mathematical expressions through natural selection, discovering symbolic formulas that fit the data — no calculus, no gradient descent, just evolution. Programs are represented as Abstract Syntax Trees (ASTs) with four node types: variables, constants, binary operators (+, -, *, /, **), and unary functions (sin, cos, abs, sqrt, ln, neg). A population of random trees is generated using ramped half-and-half initialization for diversity. Each generation, individuals are evaluated for fitness using inverse MSE with parsimony pressure that penalizes bloated expressions. Tournament selection picks parents biased toward fitter individuals. Subtree crossover swaps random branches between two parent trees (sexual reproduction of code). Point mutation replaces random nodes with fresh random subtrees (genetic variation). Elitism preserves the top performers unchanged. Bloat control rejects offspring exceeding 50 nodes. The engine includes protected operators — division by near-zero returns 1, powers are clamped, all results checked for finiteness. A seeded PRNG (Mulberry32) ensures reproducibility. In the demo, GeneForge discovers x^2+1 from 11 data points in just 8 generations (fitness 1.0, perfect), discovers z=x+2y from 49 two-variable examples (fitness 1.0, perfect), and closely approximates 2x+3. The evolution trace shows the progression from simple expressions to correct formulas. The predict() function on results enables using evolved programs as actual functions. This is real symbolic regression — the program literally writes programs by evolving them. Exports include GeneForge constructor, quickEvolve helper, all genetic operators (crossover, mutate, randomTree, fitness), and demo function for 7 total exports.
